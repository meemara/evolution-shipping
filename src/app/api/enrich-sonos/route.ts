...
// POST /api/enrich-sonos?secret=SYNC_SECRET
// Receives Sonos portal order data and enriches existing shipping tracker orders.
//
// For each Sonos order:
//   1. Match by tracking number (carrier emails from FedEx may already have the tracking)
//   2. Match by order number (Sonos order # may appear in email subjects)
//   3. Match by PO number (PO # from Sonos matches PO # parsed from emails)
//   4. Match by Sonos order number appearing in description/subject
...
export async function POST(request: Request) {
  try {
    ...
    const sonosOrders: SonosOrder[] = body.orders || [];

    if (sonosOrders.length === 0) {
    ...
    for (const sOrder of sonosOrders) {
      ...
      // Strategy 2: Match by Sonos order number appearing in existing order_number field
      ...
      // Strategy 4: Match by Sonos order number appearing in description/subject
      if (!matchedRow && sOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match Sonos/Sonos vendor + similar project name
      if (!matchedRow && sOrder.project) {
        const projLower = sOrder.project.toLowerCase();
        for (const row of existingOrders) {
          if (
            row.vendor &&
            row.vendor.toLowerCase().includes('sonos') &&
            row.project &&
      ...
      if (matchedRow) {
        // Enrich the existing order with Sonos data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'Sonos' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=Sonos');
        ...
        const description = [
          ...
          sOrder.order_number ? `Sonos #${sOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `Sonos Order ${sOrder.order_number}`;

        // Map Sonos status to shipping tracker status
        ...
        const insertResult = await sql`
          ...
          VALUES (
            ...
            ${sOrder.project},
            ${`Source: Sonos Portal | PO: ${sOrder.po_number || 'N/A'}`},
            'sonos-sync'
          )
        ...
        inserted++;
        details.push(`Inserted new: Sonos #${sOrder.order_number} (${sOrder.project || 'no project'})`);
      } else {
        skipped++;
      }
    }
    ...
    const summary = {
      ...
      totalReceived: sonosOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('Sonos enrichment error:', error);
