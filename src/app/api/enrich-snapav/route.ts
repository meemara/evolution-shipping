...
// POST /api/enrich-snapav?secret=SYNC_SECRET
// Receives SnapAV portal order data and enriches existing shipping tracker orders.
//
// For each SnapAV order:
//   1. Match by tracking number (carrier emails from FedEx may already have the tracking)
//   2. Match by order number (SnapAV order # may appear in email subjects)
//   3. Match by PO number (PO # from SnapAV matches PO # parsed from emails)
//   4. Match by SnapAV order number appearing in description/subject
...
export async function POST(request: Request) {
  try {
    ...
    const snapavOrders: SnapAVOrder[] = body.orders || [];

    if (snapavOrders.length === 0) {
    ...
    for (const sOrder of snapavOrders) {
      ...
      // Strategy 2: Match by SnapAV order number appearing in existing order_number field
      ...
      // Strategy 4: Match by SnapAV order number appearing in description/subject
      if (!matchedRow && sOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match SnapAV/Snap One vendor + similar project name
      if (!matchedRow && sOrder.project) {
        const projLower = sOrder.project.toLowerCase();
        for (const row of existingOrders) {
          if (
            row.vendor &&
            (row.vendor.toLowerCase().includes('snap') ||
             row.vendor.toLowerCase().includes('adi')) &&
            row.project &&
      ...
      if (matchedRow) {
        // Enrich the existing order with SnapAV data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'SnapAV' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=SnapAV');
        ...
        const description = [
          ...
          sOrder.order_number ? `SnapAV #${sOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `SnapAV Order ${sOrder.order_number}`;

        // Map SnapAV status to shipping tracker status
        ...
        const insertResult = await sql`
          ...
          VALUES (
            ...
            ${sOrder.project},
            ${`Source: SnapAV Portal | PO: ${sOrder.po_number || 'N/A'}`},
            'snapav-sync'
          )
        ...
        inserted++;
        details.push(`Inserted new: SnapAV #${sOrder.order_number} (${sOrder.project || 'no project'})`);
      } else {
        skipped++;
      }
    }
    ...
    const summary = {
      ...
      totalReceived: snapavOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('SnapAV enrichment error:', error);
