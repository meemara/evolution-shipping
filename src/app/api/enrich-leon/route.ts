...
// POST /api/enrich-leon?secret=SYNC_SECRET
// Receives Leon Speakers portal order data and enriches existing shipping tracker orders.
//
// For each Leon order:
//   1. Match by tracking number (carrier emails may already have the tracking)
//   2. Match by order number (Leon order ID)
//   3. Match by PO number
//   4. Match by Leon order number appearing in description/subject
...
export async function POST(request: Request) {
  try {
    ...
    const leonOrders: LeonOrder[] = body.orders || [];

    if (leonOrders.length === 0) {
    ...
    for (const lOrder of leonOrders) {
      ...
      // Strategy 2: Match by Leon order number in existing order_number field
      ...
      // Strategy 4: Match by Leon order number appearing in description
      if (!matchedRow && lOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match Leon vendor + similar project name
      if (!matchedRow && lOrder.project) {
        const projLower = lOrder.project.toLowerCase();
        for (const row of existingOrders) {
          if (
            row.vendor &&
            row.vendor.toLowerCase().includes('leon') &&
            row.project &&
      ...
      if (matchedRow) {
        // Enrich the existing order with Leon data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'Leon Speakers' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=Leon Speakers');
        ...
        const description = [
          ...
          lOrder.order_number ? `Leon #${lOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `Leon Order ${lOrder.order_number}`;

        // Map Leon status to shipping tracker status
        let trackerStatus = 'Order Confirmed';
        const statusLower = (lOrder.status || '').toLowerCase();
        if (statusLower === 'complete') trackerStatus = 'Delivered';
        else if (statusLower === 'processing') trackerStatus = 'Processing';
        else if (statusLower === 'pending') trackerStatus = 'Order Confirmed';
        ...
        const insertResult = await sql`
          ...
          VALUES (
            ...
            ${lOrder.project},
            ${`Source: Leon Speakers Portal | PO: ${lOrder.po_number || 'N/A'}`},
            'leon-sync'
          )
        ...
        inserted++;
        details.push(`Inserted new: Leon #${lOrder.order_number} (${lOrder.project || 'no project'})`);
      }
    }

    const summary = {
      timestamp: new Date().toISOString(),
      totalReceived: leonOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('Leon enrichment error:', error);
