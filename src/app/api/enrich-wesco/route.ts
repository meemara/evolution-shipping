...
// POST /api/enrich-wesco?secret=SYNC_SECRET
// Receives Wesco portal order data and enriches existing shipping tracker orders.
//
// For each Wesco order:
//   1. Match by tracking number (carrier emails from FedEx may already have the tracking)
//   2. Match by order number (Wesco order # may appear in email subjects)
//   3. Match by PO number (PO # from Wesco matches PO # parsed from emails)
//   4. Match by Wesco order number appearing in description/subject
...
export async function POST(request: Request) {
  try {
    ...
    const wescoOrders: WescoOrder[] = body.orders || [];

    if (wescoOrders.length === 0) {
    ...
    for (const sOrder of wescoOrders) {
      ...
      // Strategy 2: Match by Wesco order number appearing in existing order_number field
      ...
      // Strategy 4: Match by Wesco order number appearing in description/subject
      if (!matchedRow && sOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match Wesco/Wesco vendor + similar project name
      if (!matchedRow && sOrder.project) {
        const projLower = sOrder.project.toLowerCase();
        for (const row of existingOrders) {
          if (
            row.vendor &&
            (row.vendor.toLowerCase().includes('wesco') ||
             row.vendor.toLowerCase().includes('anixter')) &&
            row.project &&
      ...
      if (matchedRow) {
        // Enrich the existing order with Wesco data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'Wesco' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=Wesco');
        ...
        const description = [
          ...
          sOrder.order_number ? `Wesco #${sOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `Wesco Order ${sOrder.order_number}`;

        // Map Wesco status to shipping tracker status
        ...
        const insertResult = await sql`
          ...
          VALUES (
            ...
            ${sOrder.project},
            ${`Source: Wesco Portal | PO: ${sOrder.po_number || 'N/A'}`},
            'wesco-sync'
          )
        ...
        inserted++;
        details.push(`Inserted new: Wesco #${sOrder.order_number} (${sOrder.project || 'no project'})`);
      } else {
        skipped++;
      }
    }
    ...
    const summary = {
      ...
      totalReceived: wescoOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('Wesco enrichment error:', error);
