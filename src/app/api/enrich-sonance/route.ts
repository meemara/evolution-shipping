...
// POST /api/enrich-sonance?secret=SYNC_SECRET
// Receives Sonance portal order data and enriches existing shipping tracker orders.
//
// For each Sonance order:
//   1. Match by tracking number (carrier emails from FedEx may already have the tracking)
//   2. Match by order number (Sonance order # may appear in email subjects)
//   3. Match by PO number (PO # from Sonance matches PO # parsed from emails)
//   4. Match by Sonance order number appearing in description/subject
...
export async function POST(request: Request) {
  try {
    ...
    const sonanceOrders: SonanceOrder[] = body.orders || [];

    if (sonanceOrders.length === 0) {
    ...
    for (const sOrder of sonanceOrders) {
      ...
      // Strategy 2: Match by Sonance order number appearing in existing order_number field
      ...
      // Strategy 4: Match by Sonance order number appearing in description/subject
      if (!matchedRow && sOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match Sonance/Snap One vendor + similar project name
      ...
      if (matchedRow) {
        // Enrich the existing order with Sonance data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'Sonance' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=Sonance');
        ...
        const description = [
          ...
          sOrder.order_number ? `Sonance #${sOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `Sonance Order ${sOrder.order_number}`;

        // Map Sonance status to shipping tracker status
        ...
        const insertResult = await sql`
          ...
          VALUES (
            ...
            ${sOrder.project},
            ${`Source: Sonance Portal | PO: ${sOrder.po_number || 'N/A'}`},
            'sonance-sync'
          )
        ...
        inserted++;
        details.push(`Inserted new: Sonance #${sOrder.order_number} (${sOrder.project || 'no project'})`);
      } else {
        skipped++;
      }
    }
    ...
    const summary = {
      ...
      totalReceived: sonanceOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('Sonance enrichment error:', error);
