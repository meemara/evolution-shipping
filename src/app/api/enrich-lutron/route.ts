...
// POST /api/enrich-lutron?secret=SYNC_SECRET
// Receives Lutron portal order data and enriches existing shipping tracker orders.
//
// For each Lutron order:
//   1. Match by tracking number (carrier emails may already have the tracking)
//   2. Match by Lutron order number in existing order data
//   3. Match by PO number (Lutron PO # matches PO # parsed from emails)
//   4. Match by Lutron order number in description/notes
//   5. Match Lutron vendor + similar project name
//   6. If no match found but order has tracking, insert as new order

interface LutronOrder {
  vendor: string;
  order_number: string;
  po_number: string;
  project: string;
...
export async function POST(request: Request) {
  try {
    ...
    const lutronOrders: LutronOrder[] = body.orders || [];

    if (lutronOrders.length === 0) {
    ...
    for (const lOrder of lutronOrders) {
      ...
      // Strategy 2: Match by Lutron order number in existing order_number field
      if (!matchedRow && lOrder.order_number) {
        ...
        // Partial match — Lutron order number appears in existing order numbers
        if (!matchedRow) {
          for (const row of existingOrders) {
      ...
      // Strategy 3: Match by PO number in existing order data
      ...
      // Strategy 4: Match by Lutron order number in description/notes
      if (!matchedRow && lOrder.order_number) {
        for (const row of existingOrders) {
      ...
      // Strategy 5: Match Lutron vendor + similar project name
      if (!matchedRow && lOrder.project) {
        const projLower = lOrder.project.toLowerCase();
        for (const row of existingOrders) {
          if (
            row.vendor &&
            row.vendor.toLowerCase().includes('lutron') &&
            row.project &&
      ...
      if (matchedRow) {
        // Enrich the existing order with Lutron data
        ...
        const newVendor = matchedRow.vendor === 'Unknown' ? 'Lutron' : null;
        ...
        if (newProject || newTracking || newCarrier || newDelivery || newVendor) {
          ...
          if (newVendor) updates.push('vendor=Lutron');
      ...
      } else if (lOrder.tracking_number || !['Complete', 'Cancelled'].includes(lOrder.status)) {
        // No match found — insert as new order if it has useful data
        const description = [
          lOrder.po_number ? `PO: ${lOrder.po_number}` : '',
          lOrder.order_number ? `Lutron #${lOrder.order_number}` : '',
        ].filter(Boolean).join(' — ') || `Lutron Order ${lOrder.order_number}`;

        // Map Lutron status to shipping tracker status
        let trackerStatus = 'Order Confirmed';
        if (lOrder.status === 'Complete') trackerStatus = 'Delivered';
        else if (lOrder.status === 'Shipped') trackerStatus = 'Shipped';
        else if (lOrder.status === 'Partially Shipped') trackerStatus = 'Partially Shipped';
    ...
    const summary = {
      ...
      totalReceived: lutronOrders.length,
      enriched,
      inserted,
      skipped,
    ...
    console.error('Lutron enrichment error:', error);
